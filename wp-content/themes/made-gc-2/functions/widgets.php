...
if($typecount>0) {
	class oswc_tabbed_reviews extends WP_Widget {
		...
		function widget( $args, $instance ) {
			...
			<div id="tabbed-<?php echo $reviewtype; ?>-reviews" class="simple-list review-cats">
			...
			<p>
				...
				<select name="<?php echo $this->get_field_name( 'reviewtype' ); ?>">
					<?php foreach($oswcPostTypes->postTypes as $postType){
						if($postType->enabled){?>
							<option<?php if($instance['reviewtype']==$postType->safe_name) { ?> selected<?php } ?> value="<?php echo $postType->name; ?>"><?php echo ucwords($postType->name); ?></option>
						<?php }
					}?>
				</select>
			</p>
				...
				<?php foreach($oswcPostTypes->postTypes as $postType){ 
					...
					<div id="tabs-<?php echo $postType->id; ?>-<?php echo $sort; ?>" class="tabdiv">
						<ul>
	                        ...
	                        <li class="more" title="View all <?php echo $reviewType->name; ?> reviews"><a href="<?php echo $reviewType->more_link; ?>"><?php _e( 'More','made'); ?></a></li>
                            <li class="last">&nbsp;</li>
            ...
            <div class="complex-list <?php echo $cssclass; ?><?php if($thumbsize=="footer-thumbnail") { ?> small<?php } ?>">
				...
				if ( $title ) { ?>                	
					<?php echo $before_title; ?>
                    	...
                    	<?php if($reviewtype!="All Reviews") { ?>
                    
                    		<a class="more" href="<?php echo $postType->more_link ?>" title="<?php _e('View all ','made'); ?><?php echo $postType->name; ?><?php _e(' reviews','made'); ?>">&nbsp;</a>
