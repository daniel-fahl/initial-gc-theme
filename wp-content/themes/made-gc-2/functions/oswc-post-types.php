...
class OSWCTaxonomy{
	...
	function __construct($postTypeName, $name, $isPrimary = NULL, $includeInExcerpt = NULL, $id = NULL, $slug = NULL, $url_name = NULL){
		$this->name = $name;
		...
		if(!isset($url_name)){
			...
			//$this->url_name = strtolower(str_replace("/", "-", str_replace(" ","-",str_replace("&","-",str_replace("'","",$name)))));
		}else{
...
class oswcPostTypes {
	...
	public function the_taxonomies($postType, $getExcerptOnly = NULL){
		...
		foreach($taxonomies as $taxonomy){
			echo get_the_term_list( $post->ID, $taxonomy->id, '<span class="taxName">' . __(ucwords($taxonomy->name),'made') . '</span>: <span class="taxContent">', ', ', '</span>' );
