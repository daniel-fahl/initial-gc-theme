...
// Post Meta Fields
function get_custom_field_2($key, $echo = FALSE) {
	global $post;
...
if ( function_exists( 'add_theme_support' ) ) { // Added in 2.9
	...
	add_image_size( 'featured-full', 1000, 350, true );	
	...
	add_image_size( 'single-mustread', 1000, 350, true );
	add_image_size( 'news', 80, 80, true );
...
function additional_fields () {
	...
	if($userRatingsEnabled) {
		...
		if(strpos($meta,$ip) === false) {
			...
			echo '<div class="comment_rating_label label"><label for="rating">'. __('My Rating') . '</label></div>';
...
function better_human_time_diff( $from, $to = '' ) {
	if ( empty($to) )
		$to = time();
	...
	if ($diff <= 3600) {
		...
		$since = sprintf(_n('%s <span>min</span>', '%s <span>mins</span>', $mins), $mins);
	} else if (($diff <= 86400) && ($diff > 3600)) {
		$hours = round($diff / 3600);
		if ($hours <= 1) {
			$hours = 1;
		}
		$since = sprintf(_n('%s <span>hour</span>', '%s <span>hours</span>', $hours), $hours);
	} elseif ($diff >= 86400) {
		$days = round($diff / 86400);
		if ($days <= 1) {
			$days = 1;
		}
		$since = sprintf(_n('%s <span>day</span>', '%s <span>days</span>', $days), $days);
	}
	return $since;
}
...
<table class="form-table">
<tr>
<th><label for="address"><?php _e("Job Description"); ?></label></th>
<td>
<input type="text" name="address" id="address" value="<?php echo esc_attr( get_the_author_meta( 'address', $user->ID ) ); ?>" class="regular-text" /><br />
<span class="description"><?php _e("Job Description"); ?></span>
</td>
</tr>
</table>
<?php }
