...
function theme_reviews_options() {
	...
	and not somewhere else, very important for security */ ?>
        ...
        <div style="padding:15px 15px 20px 15px;background:#F0F6FF;border:1px solid #CCC;margin:0px 0px 25px 0px;width:820px;">
            ...
            <td>
            <input id="review_types" name="oswc_reviews[review_types]" type="text" value="<?php if(!empty($settings['review_types'])) { esc_attr_e($settings['review_types']);} ?>" style="width:580px;padding:7px 15px;background:#FFF;color:#000;font-size:1.5em;border:1px solid #666;font-family:'Lucida Sans Unicode', 'Lucida Grande', sans-serif;" />
            <div style="color:#777;">
									...
									<div class="rm_options">
										...
										<div style="float:left;">
											...
											for($i = 1; $i <= $num; $i++) {
												...
												?>
													<?php if($i==1) { ?>
                                                    	<?php _e( 'Primary:','made' ); ?><br />
                                                    <?php } elseif($i==2) { ?>
                                                    	<?php _e( 'Secondary:','made' ); ?><br />
                                                    <?php } ?>
                                                    <input id="taxonomy_<?php echo $i; ?>_<?php echo $reviewType; ?>" name="oswc_reviews[taxonomy_<?php echo $i; ?>_<?php echo $reviewType; ?>]" type="text" value="<?php echo $taxonomyName; ?>" style="width:200px;" /><br />
...
function oswc_validate_reviews( $input ) {
	...
	foreach($reviewTypes as $reviewTypeName) { //do this for each review type
		...
		for($i = 1; $i <= $num; $i++) {
			...
			if($i==1) {
				array_push($input['taxonomies_'.$reviewType],new OSWCTaxonomy($reviewType, $taxonomyName, true, true));
			} else {
				array_push($input['taxonomies_'.$reviewType],new OSWCTaxonomy($reviewType, $taxonomyName));//was this accidentaly a meta field instead of a taxonomy? ROFL, YES!!!!
			}
		}
	}
