...
if($oswc_archive_header=="") {
	...
	if (is_category()) {
		...
		$oswc_archive_header = __("Archive for ".date_i18n('F jS, Y'), "made");
	} elseif (is_month()) {
		$oswc_archive_header = __("Archive for ".date_i18n('F, Y'), "made");
	} elseif (is_year()) {
		$oswc_archive_header = __("Archive for ".date_i18n('Y'), "made");
	} elseif (is_author()) {
...
<div class="main-content<?php if($oswc_archive_sidebar_show) { ?>-left<?php } ?>"> 
	...
	<?php if ( is_author()) { ?>
		...
		<div class="author_page_box clearfix">
			<?php oswc_get_template_part('authorbox'); ?>
		</div>
