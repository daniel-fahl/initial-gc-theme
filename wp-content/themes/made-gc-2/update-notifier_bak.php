...
define( 'NOTIFIER_THEME_NAME', 'Made' ); // The theme name
define( 'NOTIFIER_THEME_FOLDER_NAME', 'made' ); // The theme folder name
define( 'NOTIFIER_XML_FILE', 'http://www.brianmcculloh.com/updates/made/notifier.xml' ); // The remote notifier XML file containing the latest version of the theme and changelog
...
function update_notifier_menu() {  
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		if( (float)$xml->latest > (float)$theme_data['Version']) { // Compare current theme version with the remote XML version
			add_theme_page( NOTIFIER_THEME_NAME . ' Theme Updates', NOTIFIER_THEME_NAME . ' <span class="update-plugins count-1"><span class="update-count">1</span></span>', 'manage_sites', 'theme-update-notifier', 'update_notifier');
		}
	}	
}
...
function update_notifier_bar_menu() {
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		if( (float)$xml->latest > (float)$theme_data['Version']) { // Compare current theme version with the remote XML version
			$wp_admin_bar->add_menu( array( 'id' => 'update_notifier', 'title' => '<span>' . NOTIFIER_THEME_NAME . ' <span id="ab-updates">1</span></span>', 'href' => get_admin_url() . 'themes.php?page=theme-update-notifier' ) );
		}
	}
}
...
function update_notifier() { 
	...
	<div class="wrap">
		...
		<img style="float: left; margin: 0 20px 20px 0; border: 1px solid #ddd;" src="<?php get_template_directory_uri() . '/screenshot.png'; ?>" />
		...
		<div id="instructions">
		    ...
		    <p>Extract the zip's contents, look for the extracted theme folder, and after you have all the new files upload only the ones that were changed according to the <a href="http://support.industrialthemes.com/viewtopic.php?f=6&t=2938">update log</a> using FTP to the <strong>/wp-content/themes/<?php echo NOTIFIER_THEME_FOLDER_NAME; ?>/</strong> folder overwriting the old ones (this is why it's important to backup any changes you've made to the theme files).</p>
